...
#'                     I_human = 0.05,
#'                     boost = 0,
#'                     beta_wastewater = 0.02,
#'                     beta_food_waste = 0.000001,
#'                     beta_feed_pile = 0.0001)
#'
#' run_direct_and_indirect(iter = nsamples,
...
#' name = "Test")}
#'
run_direct_and_indirect <- function(iter = NULL, initial_compartments = NULL, 
                                    initial_compartments_steady = NULL, 
                                    params = NULL, times = NULL, name = NULL)

{  if(is.null(iter)) {stop("User must specify number of iterations - 
                           often stored as nsamples object")}
   if(is.null(initial_compartments)) {stop("User must specify initial 
                                           compartment sizes, either by hand or 
                                           with initial_compartments function")}
   if(is.null(params)) {stop("User must specify all parameters for ODE equation,
                             either by hand or with alternative function")}
   if(is.null(name)) {stop("User must name context for data storage")}

   #Establish objects
   run_id <- NULL
   initial <- initial_compartments
   initial_steady <- initial_compartments_steady
   
   tibble(run_id = 1:iter,
          inits.fall = purrr::map(run_id, ~ initial %>% map_dbl(., .x)),
          inits.steady = map(run_id, ~ initial_steady %>% map_dbl(., .x)),
          params = map(run_id, ~ params %>% map_dbl(., .x))) %>%
        mutate(ode_proj = pmap(list(y = inits.fall, parms = params), 
                          deSolve::ode, times = times, 
                          func = direct_and_indirect_sirs_with_cumulative)) %>%
        mutate(steady_state = pmap(list(y = inits.steady, parms = params), 
                              rootSolve::runsteady, 
                              func = direct_and_indirect_sirs, mf = 10)) %>%
