...
#'
#' `direct_and_indirect_sir` is written to work alongside the `deSolve::ode()` function, and
...
#' `direct_and_indirect_sir` instead. The parameter `boost` is included in
...
#' `Management_Alternative_Systems.Rmd` from the whitetailedSIRS package to see an example of its use. In all
...
#' parms = example_params, func = direct_and_indirect_sir)
#' }
#'
#'
direct_and_indirect_sirs <- function(time, state, parameters){
   with(as.list(c(state, parameters)), {
      ...
      dS_wild <- alpha_immunity * R_wild -
         (S_wild * ((beta_aero_ww * I_wild) + (beta_dc_ww * I_wild) +
                       ...
                       (beta_aero_hw * I_human) + beta_wastewater + (beta_food_waste * I_human) + (beta_feed_pile * I_wild)))
