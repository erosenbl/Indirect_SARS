...
#' that include both direct and indirect transmission pathways
#'
...
#' events per day).
#' @param omega_wastewater Contact rate between contaminated water and deer
#' @param omega_foodwaste Contact rate between contaminated food waste and deer
#' @param omega_feedpile Contact rate between contaminated feed pile and deer 
...
#' @param sigma_wastewater Probability of infection of a deer drinking 
#' contaminated waste water
#' @param sigma_foodwaste Probability of infection of a deer eating 
#' contaminated food waste
#' @param sigma_feedpile Probability of infection of a deer eating from a
#' contaminated feed pile
...
#' omega_hw = rep(0.01,5),
#' omega_hc = rep(0.25,5),
#' omega_wastewater = rep(0.005,5),
#' omega_foodwaste = rep(0.004,5),
#' omega_feedpile = rep(0.005,5),
...
#' sigma_wastewater = rep(0.02,5),
#' sigma_foodwaste = rep(0.02,5),
#' sigma_feedpile = rep(0.02,5),
#' gamma_recov = rep(1/6,5),
#' I_human = rep(0.05,5),
#' boost = rep(0,5))}
#'
alternative_indirect_and_direct <- function(alpha_immunity = NULL, 
                                            omega_ww = NULL, omega_cw = NULL, 
                                            omega_cc = NULL, omega_hw = NULL, 
                                            omega_hc = NULL, 
                                            omega_wastewater = NULL, 
                                            omega_foodwaste = NULL, 
                                            omega_feedpile = NULL, 
                                            ...
                                            sigma_wastewater = NULL, 
                                            sigma_foodwaste = NULL, 
                                            sigma_feedpile = NULL, 
                                            gamma_recov = NULL, 
                                            I_human = NULL, boost = NULL)

{
   if(is.null(alpha_immunity)) {stop("Values for alpha_immunity (duration of 
                                     temporary immunity to SARS-CoV-2) is 
                                     required") }
   if(is.null(omega_ww)) {stop("Values for omega_ww (proximity rate between 
                               wild deer) is required") }
   if(is.null(omega_cw)) {stop("Values for omega_cw (fenceline proximity rate 
                               between captive and wild deer) is required") }
   if(is.null(omega_cc)) {stop("Values for omega_cc (proximity rate between 
                               captive deer) is required") }
   if(is.null(omega_hw)) {stop("Values for omega_hw (proximity rate between 
                               humans and wild deer) is required") }
   if(is.null(omega_hc)) {stop("Values for omega_hc (proximity rate between 
                               humans and captive deer) is required") }
  if(is.null(omega_wastewater)) {stop("Values for omega_cc (proximity rate 
                                      between deer and wastewater) is 
                                      required") }
  if(is.null(omega_foodwaste)) {stop("Values for omega_hw (proximity rate 
                                     between deer and foodwaste) is required") }
  if(is.null(omega_feedpile)) {stop("Values for omega_hc (proximity rate 
                                    between deer and feed piles) is required") }
   if(is.null(sigma_aero_deer_deer_wild)) {stop("Values for derived 
                                                sigma_aero_deer_deer_wild 
                                                (probability of infection via 
                                                aerosol transmission in wild 
                                                deer) is required") }
   if(is.null(sigma_aero_deer_deer_captive)) {stop("Values for derived 
                                                   sigma_aero_deer_deer_captive 
                                                   (probability of infection via
                                                   aerosol transmission in 
                                                   captive deer) is required") }
   if(is.null(sigma_aero_deer_human_wild)) {stop("Values for derived 
                                                 sigma_aero_deer_human_wild 
                                                 (probability of infection via 
                                                 aerosol transmission from 
                                                 humans to wild deer) is 
                                                 required") }
   if(is.null(sigma_aero_deer_human_capt)) {stop("Values for derived 
                                                 sigma_aero_deer_human_capt 
                                                 (probability of infection via 
                                                 aerosol transmission from 
                                                 humans to captive deer) is 
                                                 required") }
   if(is.null(sigma_wastewater)) {stop("Values for derived sigma_wastewater 
                                       (probability of infection via wastewater)
                                       is required") }
   if(is.null(sigma_foodwaste)) {stop("Values for derived sigma_foodwaste 
                                      (probability of infection via foodwaste) 
                                      is required") }
   if(is.null(sigma_feedpile)) {stop("Values for derived sigma_feedpile 
                                     (probability of infection via feedpile) is 
                                     required") }
   if(is.null(epsilon_dc)) {stop("Values for epsilon_dc (probability of direct 
                                 contact between deer, given proximity) is 
                                 required") }
   if(is.null(sigma_dc_deer_deer)) {stop("Values for nu_domega_deer_deer 
                                         (probability of infection via fluid 
                                         transmission between deer) is 
                                         required") }
   if(is.null(gamma_recov)) {stop("Values for gamma_recov (duration of recovery 
                                  from infection) is required") }
   if(is.null(I_human)) {stop("Values for I_human (prevalence in human 
                              population) is required") }
   if(is.null(boost)) {stop("Values for boost (boosting rate in captive deer) 
                            is required") }

   list(
      alpha_immunity = alpha_immunity,
      beta_aero_ww = omega_ww * sigma_aero_deer_deer_wild,
      beta_aero_cw = omega_cw * sigma_aero_deer_deer_wild,
      ...
      beta_wastewater = omega_wastewater * sigma_wastewater,
      beta_food_waste = omega_foodwaste * sigma_foodwaste,
      beta_feed_pile = omega_feedpile * sigma_feedpile,
